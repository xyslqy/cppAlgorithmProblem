#include<bits/stdc++.h>
using namespace std;
const int N=1e7+10,INF=0x7fffffff;
int read(){
	int x=0;
	char c=getchar();
...
int main(){
	...
	while(m--){
		...
		if(o==1) ins(root,x);
		if(o==2) rm(root,x);
		if(o==3) last=rnk(root,x)-1,ans^=last;
		if(o==4) last=val(root,x+1),ans^=last;
		if(o==5) last=pre(x),ans^=last;
		if(o==6) last=nxt(x),ans^=last;
	}
	cout<<ans;
	return 0;
} 
